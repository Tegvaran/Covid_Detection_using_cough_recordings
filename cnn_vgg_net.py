...
# Change to the right Directory Path
DATA_PATH = "C:/Users/tommy/Documents/rnn_model/spectrum_test/data.json"
...
def build_model(input_shape):
    ...
    model = Sequential()
    # conv_1
    model.add(Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv1', input_shape=input_shape))#batch__input_shape=(224,224,3)
...
if __name__ == "__main__":
    ...
    # pick a sample to predict from the test set for healthy
    index_1 = np.where(y_test==1)
    X_to_predict = X_test[index_1[0][0]]
    y_to_predict = y_test[index_1[0][0]]

    # predict sample
    predict(model, X_to_predict, y_to_predict)

    # pick a sample to predict from the test set for covid
    index_0 = np.where(y_test==0)
    X_to_predict = X_test[index_0[0][0]]
    y_to_predict = y_test[index_0[0][0]]
